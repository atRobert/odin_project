...
async function getCityWeather(cityName){
    let long 
    let lat
    const weather = await fetch(`http://api.openweathermap.org/data/2.5/weather?q=${cityName}&units=imperial&APPID=fd44eb02e519e736eeb68874e1c597c9`)
    const data = await weather.json()
    
    long = data.coord.lon
    lat = data.coord.lat
    const time = await fetch(`http://api.timezonedb.com/v2.1/get-time-zone?key=BX35L7KUUB8G&format=json&by=position&lat=${lat}&lng=${long}`)
